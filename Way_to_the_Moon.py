...
m = (100 + 5.5 + 22.5 + 15) * 10 ** 3  # Масса ракеты в начале перелёта, кг.
...
def flying(r_pos, v, m_pos, mass, fx, fy, detachment):
...
if __name__ == '__main__':
    ...
    ax2.set_ylabel(r'$y$, м')
    ax2.plot([c_in_m_fr[i][0] for i in range(len(c_in_m_fr))], [c_in_m_fr[i][1] for i in range(len(c_in_m_fr))])
    ax3 = fig.add_subplot(223)
    ax3.set_title(r'Скорость ракеты в СО, связанной с Землёй')
    ax3.set_xlabel(r'$v_x$, м/c')
    ax3.set_ylabel(r'$v_y$, м/c')
